...
class QueueCog(MusicCog):
    ...
    async def remove(
        ...
        try:
            ...
            await self.warn(interaction, f"유효하지 않은 순번입니다. (최대: {player.queue_size})")
            return
        await self.respond(
            interaction,
            self.embeds.success(f"**{truncate_string(removed.title, 40)}** 제거되었습니다."),
        )
    ...
    async def clear(self, interaction: discord.Interaction) -> None:
        ...
        if count > 0:
            await self.respond(interaction, self.embeds.success(f"대기열에서 **{count}곡**을 제거했습니다."))
        else:
            await self.warn(interaction, "대기열이 이미 비어있습니다.")
    ...
    async def shuffle(self, interaction: discord.Interaction) -> None:
        ...
        if count > 0:
            await self.respond(interaction, self.embeds.success(f"{Emoji.SHUFFLE} 대기열의 **{count}곡**을 섞었습니다."))
        else:
            await self.warn(interaction, "대기열에 곡이 부족합니다.")
