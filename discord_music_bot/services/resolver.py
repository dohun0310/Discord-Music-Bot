...
def _entry_to_track(entry: Optional[dict[str, Any]], requester: str) -> Optional[Track]:
    if not entry:
        return None
...
def build_resolution(
    ...
    if "entries" in data:
        valid = [t for t in (_entry_to_track(e, requester) for e in data["entries"]) if t]
        if not valid:
            return None
        if is_search:
...
def build_batch(data: dict[str, Any], requester: str) -> list[Track]:
    """플레이리스트 다음 배치(dict)를 Track 리스트로 변환한다 (순수)."""
    if "entries" not in data:
        return []
    return [t for t in (_entry_to_track(e, requester) for e in data["entries"]) if t]
...
class YtDlpTrackResolver:
    ...
    async def resolve(self, query: str, requester: str) -> Optional[ResolveResult]:
        ...
        if data is None:
            return None
        return build_resolution(data, query=query, requester=requester, is_search=is_search)
    ...
    async def load_playlist_batch(
        ...
        if data is None:
            return []
        return build_batch(data, requester)
